...
(function installProxy(){
  ...
  window._callClaudeViaProxy=async function(sys,msg,maxTok){
    ...
    const res=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json','apikey':_key(),'Authorization':session?.access_token?'Bearer '+session.access_token:''},body:JSON.stringify({model:'gpt-4o',max_tokens:maxTok||2000,system:sys,messages:[{role:'user',content:msg}]})});
